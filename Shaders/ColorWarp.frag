    ...
    void frag(out vec4 fragColor, in vec2 fragCoord){
        ...
        float warp1 = abs(dualWarp(uv * scale, 0.1, q, r));

        float warp2 = fbm(vec2(dualWarp(uv * scale, 0.4, p, g),dualWarp(uv * scale, 0.1, p, g)),0.5);

        float gain = 5.0;

        float min = 0.1;
        vec3 col = vec3(r.x * uv.y * iRands.x,r.y * uv.x * iRands.y,q.x * q.y * iRands.z);
        vec3 col2 = vec3(q.y * uv.x * iRands.w,q.x * uv.y * iRands.z,r.x * r.y * iRands.y);
        vec3 col3 = vec3(uv.x,uv.y,(p.x * p.y + g.x * g.y) * iRands.x);

        col *= warp1 * gain;
        col = mix(col, col2, smoothstep(q.x,r.x,fbm(vec2(q.y,r.y),0.9)));
        col = mix(col, col3 * warp2, smoothstep(q.y,r.y,fbm(vec2(q.x,r.x),0.1)));
        col += min;

        vec4 clipped = vec4(col,g.y * 0.5);

        vec4 clipped2 = vec4(col,g.x * 0.5);

        vec4 bg = vec4(0.0,0.0,0.0,1.0);

        vec4 sig = bg + (clipped + clipped2);
